...
% Decomposed Plots
if exist('xEBSD','var')
    plot_DecomposedStess(sXXd,sYYd,sZZd,sXYd,sXZd,sYZd,Maps,Saf);
    if isfield(Maps,'SavingD')
        saveas(gcf, [fileparts(Maps.SavingD) '\Stress_I_II_III.fig']);
        saveas(gcf, [fileparts(Maps.SavingD) '\Stress_I_II_III.tif']);  close
    end
else
...
axis image; axis off; colormap jet; box off; %set(gca,'Ydir','reverse')
c  =colorbar;	cU(2,:) = c.Limits;     colorbar off;
s3=subplot(3,3,3);  	contourf(Maps.X,Maps.Y,Maps.S31*Saf*1e-9,'LineStyle','none');
...
axis image; axis off; colormap jet; box off; %set(gca,'Ydir','reverse')
c  =colorbar;	cU(4,:) = c.Limits;     colorbar off;
s6=subplot(3,3,6);  	contourf(Maps.X,Maps.Y,Maps.S32*Saf*1e-9,'LineStyle','none');
